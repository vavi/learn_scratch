...
public final class Unsafe {
	 ...
	 * previous release but is now a wrapper which simply casts the offset to a
	 * long value. It provides backward compatibility with bytecodes compiled
	 * against 1.4.
	 * 
	 ...
	 * unsigned number to a Java long. The pointer may be indexed by any given
	 * byte offset, simply by adding that offset (as a simple integer) to the
	 * long representing the pointer. The number of bytes actually read from the
	 ...
	 * The resulting native pointer will never be zero, and will be aligned for
	 ...
	 * resulting native pointer will be aligned for all value types. Dispose of
	 * this memory by calling {@link #freeMemory}, or resize it with
	 * {@link #reallocateMemory}. The address passed to this method may be null,
	 ...
	 * {@link #putAddress}. This value will be either 4 or 8. Note that the
	 * sizes of other primitive types (as stored in native memory blocks) is
