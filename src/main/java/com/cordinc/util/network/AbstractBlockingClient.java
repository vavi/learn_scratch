...
public abstract class AbstractBlockingClient implements Runnable {
	 ...
	 */
	public AbstractBlockingClient(InetAddress server, int port,
			MessageLength messageLength, int defaultBufferSize) {
		this.server = server;
		this.port = port;
	...
	public void run() {
		...
		try {
			...
			while (state.get() == State.RUNNING) {
				limit += in.get()
						.read(inBuffer, limit, inBuffer.length - limit);
				if (limit >= byteLength) {
					int messageLen;
					do {
						byte[] lengthBytes = new byte[byteLength];
						System.arraycopy(inBuffer, 0, lengthBytes, 0,
								byteLength);
						messageLen = (int) messageLength
								.bytesToLength(lengthBytes);
						if (limit >= messageLen) {
							// enough data to extract the message
							byte[] message = new byte[messageLen];
							System.arraycopy(inBuffer, byteLength, message, 0,
									messageLen);
							messageReceived(ByteBuffer.wrap(message));
							// compact inBuffer
							byte[] temp = new byte[inBuffer.length];
							System.arraycopy(inBuffer, 0, temp, messageLen
									+ byteLength, limit - messageLen
									- byteLength);
							inBuffer = temp;
							limit = limit - messageLen - byteLength;
						} else if (messageLen > inBuffer.length) {
							byte[] temp = new byte[messageLen + byteLength];
							System.arraycopy(inBuffer, 0, temp, 0,
									inBuffer.length);
							inBuffer = temp;
						}
