...
/**
 * Encodes a message length in 2 bytes, thus the maximum message length is 65535
 * bytes.
 */
public final class TwoByteMessageLength implements MessageLength {
	...
	public long bytesToLength(byte[] bytes) {
		if (bytes.length != NUM_BYTES) {
			throw new IllegalStateException("Wrong number of bytes, must be "
					+ NUM_BYTES);
		}
	...
	public byte[] lengthToBytes(long len) {
		if (len < 0 || len > MAX_LENGTH) {
			throw new IllegalStateException(
					"Illegal size: less than 0 or greater than " + MAX_LENGTH);
		}
