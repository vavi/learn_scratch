...
public class AbstractBlockingClientTest {
	...
	@Test
	public void testReadSplitMessage() throws Exception {
		final CountDownLatch disconnected = new CountDownLatch(1);
		final CountDownLatch connected = new CountDownLatch(1);
		final AtomicReference<String> messageHolder = new AtomicReference<String>();
		...
		((SocketChannel) server.lastConnectedClient.get()).write(ByteBuffer
				.wrap(new byte[] { 0, 6 }));
		((SocketChannel) server.lastConnectedClient.get()).write(ByteBuffer
				.wrap("Hel".getBytes()));
		((SocketChannel) server.lastConnectedClient.get()).write(ByteBuffer
	...
	@Test
	public void testReadOverBufferLengthMessage() throws Exception {
		final CountDownLatch disconnected = new CountDownLatch(1);
		final CountDownLatch connected = new CountDownLatch(1);
		final AtomicReference<String> messageHolder = new AtomicReference<String>();
		final AtomicReference<CountDownLatch> latch = new AtomicReference<CountDownLatch>(
	...
	@Test
	public void testReadMultipleMessage() throws Exception {
		final CountDownLatch disconnected = new CountDownLatch(1);
		final CountDownLatch connected = new CountDownLatch(1);
		final AtomicReference<String> messageHolder = new AtomicReference<String>();
		final AtomicReference<CountDownLatch> latch = new AtomicReference<CountDownLatch>(
	...
	@Test
	public void testWriteBeforeStart() throws Exception {
		final CountDownLatch disconnected = new CountDownLatch(1);
		final CountDownLatch connected = new CountDownLatch(1);
		client = new AbstractBlockingClient(SERVER, PORT) {
			@Override
